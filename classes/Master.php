...
Class Master extends DBConnection {
	...
	function save_archive(){
		...
		if($save){
			...
			$resp['id'] = $aid;
			if(empty($id))
				$resp['msg'] = "Thesis was successfully submitted";
			else
				$resp['msg'] = "Thesis details was updated successfully.";
	...
	function award_status(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `archive_list` set award_status  = '{$award_status}' where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = "Archive award has successfully updated.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occurred. Error: " .$this->conn->error;
		}

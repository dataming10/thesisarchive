...
Class Users extends DBConnection {
	...
	public function deact_user(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `student_list` set `status` = 0 where id = $id");
		if($update){
			$this->settings->set_flashdata('success','Student Account has deactivated successfully.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
		}
